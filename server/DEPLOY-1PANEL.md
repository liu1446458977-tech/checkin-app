...
```
http://你的域名或IP:8787/          ← 看板页面
http://你的域名或IP:8787/api/health
```
...
| App 提示「连不上服务器」 | 地址写成 `http://你的域名或IP:8787`（**不要带面板的路径**，那是另一个入口）；确认手机用的是能访问外网的网络 |
...
4. 验证：浏览器打开 `http://你的域名或IP:8787/api/health` 应返回 `{"ok":true}`；
...
地址：`http://你的域名或IP:8787/admin`。**不配置管理口令 = 页面和接口一律 404（相当于不存在）**。
